...
const Input = React.forwardRef(
  (
    ...
    return (
      <>
        <div
          className={`${className} flex items-center justify-center text-gray-500 tracking-[-0.50px] text-lg bg-white-A700 ${shapes[shape] || ""} ${variants[variant]?.[color] || variants[variant] || ""} ${sizes[size] || ""}`}
        >
          {!!label && label}
          {!!prefix && prefix}
